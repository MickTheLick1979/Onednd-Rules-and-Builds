#!/usr/bin/env python3
# Phase 9: validate rules/2024/action.json against schema/2024/v1/rules.action.schema.json
...
def main() -> int:
    ...
    schema = repo / "schema/2024/v1/rules.action.schema.json"
    target = repo / "rules/2024/action.json"

    if not schema.is_file():
        print(f"[ERROR] Schema missing: {schema}", file=sys.stderr)
        return 2
    if not target.is_file():
        print(f"[ERROR] Target JSON missing: {target}", file=sys.stderr)
        return 3
    ...
    print("[OK] rules/2024/action.json conforms to rules.action schema.")
    return 0
