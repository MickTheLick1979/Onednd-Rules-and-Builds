...
DATA   = os.path.join(REPO, "rules", "2024", "class.json")
SCHEMA = os.path.join(REPO, "schema", "2024", "v1", "class.schema.json")
...
def main():
    ...
    if errors:
        sys.stderr.write(ascii(f"[FAIL] class.json has {len(errors)} validation error(s):\n"))
        for i, err in enumerate(errors, 1):
            loc = "$" + "".join(f"[{repr(p)}]" if isinstance(p, int) else f".{p}" for p in err.path)
            sys.stderr.write(ascii(f"  {i}. {loc}: {err.message}\n"))
        sys.exit(1)

    sys.stdout.write(ascii("[OK] class.json passed schema validation.\n"))
    sys.exit(0)
